...
def detect(save_img=False):
    ...
    if webcam:
        ...
        save_img = True
        # 如果检测视频的时候想显示出来，可以在这里加一行 view_img = True
        # view_img = True
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        # 设置打印信息(图片长宽)
            ...
            if len(det):
                ...
                # Print results
                # 打印检测到的类别数量
                for c in det[:, -1].unique():#unique去重
...
class detectapi:
    def __init__(self, weights, img_size=640):
        ...
        # weights, imgsz = self.opt.weights, self.opt.img_size   ########错误代码 weights 和 imgsz 是私有属性，需要加self变成共有
